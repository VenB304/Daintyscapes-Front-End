...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_charm`(
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_customization`(
    IN p_buyer_id INT,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_customization_charm`(
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_order_detail`(
    IN p_order_id INT,
    IN p_product_id INT,
    IN p_customization_id INT,
    IN p_charm_id INT,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_order_status`(IN p_status_name VARCHAR(255), OUT p_status_id INT)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_product_category`(
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_product_variant`(
    IN p_product_id INT,
    IN p_variant_name VARCHAR(50),
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `debug_add_seller_with_hash`(
	IN p_username VARCHAR(50),
	IN p_password_hash VARCHAR(100)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `debug_change_buyer_into_admin`(
    IN p_buyer_id INT,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `debug_change_buyer_into_seller`(
    IN p_buyer_id INT,
    IN p_new_username VARCHAR(50)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `debug_show_buyer_details`()
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_buyer`(
	IN p_buyer_id INT
)
BEGIN
	DECLARE v_user_id INT;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_buyer_orders`(
    IN p_username VARCHAR(50)
)
BEGIN
    SELECT 
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_buyer_profile_by_username`(
	IN p_username VARCHAR(50)
)
BEGIN
    SELECT 
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_seller_products`()
BEGIN
    SELECT 
        p.product_id, 
        p.product_name, 
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `initialize_add_order_status`()
BEGIN
	START TRANSACTION;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `modify_charm`(
    IN p_charm_id INT,
    IN p_charm_name VARCHAR(30),
    IN p_charm_base_price DECIMAL(19,4),
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `remove_product`(IN p_product_id INT)
BEGIN
    START TRANSACTION;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `remove_product_variant`(
    IN p_product_id INT
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_buyer_credentials`(
    IN p_username VARCHAR(50),
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_order_status`(IN p_order_id INT, IN p_status_id INT)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_user_credentials`(
    IN p_username VARCHAR(50),
    IN p_password_hash VARCHAR(255),
    IN p_user_id INT
)
